...
class Avatar:
    def __init__(self, neopixel_pin_1, neopixel_pin_2, neopixel_pin_3, neopixel_pin_4, button_pin, sda_pin, scl_pin,
        ...
        gc.collect()
        print("[INIT] Free RAM after collect:", gc.mem_free())
        ...
        self.green_color_list = [
            (0, 102, 0), (3, 115, 6), (6, 128, 14), (10, 141, 22),
            (13, 154, 29), (15, 168, 37), (16, 182, 45), (17, 196, 52),
            (16, 211, 60), (14, 225, 68), (9, 240, 77), (0, 255, 85)
        ]
        self.led_index = 0
        ...
        gc.collect()
        print("[INIT DONE] Free RAM:", gc.mem_free())

    def run(self):
        gc.collect()
        print("[RUN START] Free RAM:", gc.mem_free())

        if self.begin():
            print("Error in initialization. Stopping program.")
            return
    ...
    def send_msg(self, msg):
        try:
            self.client.publish(self.AIO_PLAYER_NUM, str(msg))
    ...
    def begin(self):
        gc.collect()
        print("[BEGIN] Starting setup. Free RAM:", gc.mem_free())
...
def main():
    game = Avatar(18, 19, 17, 27, 13, 21, 22, 2, info_path="info.json")
    game.run()
